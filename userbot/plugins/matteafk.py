...
async def _(event):
    ...
    if not USER_AFK:  # pylint:disable=E0602
        ...
        if reason:
            await bot.send_message(event.chat_id, f"**Ora vado AFK!** __MOTIVO ~ {reason}__")
        else:
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"**Sono offline da** `{total_afk_time}`\n**Quindi cortesemente non spammare🔇.**" + \
            f"\n\n**Quando verrò online, risponderò a tutti!🔜.**\n**MOTIVO**: {reason}" \
            if reason \
            else f"** Al momento sono offline🔇**\n\n**Lascia un messaggio, ti risponderò presto🔜**\n**Cortesemente, non spammare!✖️**"
